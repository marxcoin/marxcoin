...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MarxCoin: starting difficulty is 1 / 2^12
...
static const int64 nTargetTimespan = 2 * 60 * 60; // MarxCoin: every 2 hours
static const int64 nTargetSpacing = 120; // MarxCoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // MarxCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal marxcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MarxCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "You have nothing to lose but your chains.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // MarxCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
